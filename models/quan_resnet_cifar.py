...
class ResNetBasicblock(nn.Module):
  ...
  """
    ...
    def forward(self, x, nolast=False):
        ...
        # print("size:", t1*t2*t3)
        if nolast:
            return F.relu(residual + basicblock, inplace=True), self.output(residual + basicblock,nolast=True)
...
class CifarResNet(nn.Module):
  ...
  """
    ...
    """
        ...
        self.inplanes = 16
        self.branch_linearshape = [16384, 16384, 16384, 4096, 4096, 4096, 1024, 1024, 1024, 1024]
        self.b_index = 0
    ...
    def forward(self, x, nolast=False):
        ...
        for g in range(1, 4):
          ...
          for i in range(layer_num):
            x, branch_out = getattr(self, 'group{}'.format(g))[i](x,nolast)
...
class CifarResNet1(nn.Module):
  ...
  """
    ...
    Args:
      ...
      base_width: base width
    """
        super(CifarResNet1, self).__init__()
    ...
    def forward(self, x, nolast=True):
        x = self.conv_1_3x3(x)
        x = F.relu(self.bn_1(x), inplace=True)
        output_branch = []
        for g in range(1, 4):
...
def resnet20_quan1(num_classes=10):
    """Constructs a ResNet-20 model for CIFAR-10 (by default)
  Args:
    num_classes (uint): number of classes
  """
    model = CifarResNet1(ResNetBasicblock, 20, num_classes)
    return model
...
def resnet32_quan1(num_classes=10):
    """Constructs a ResNet-32 model for CIFAR-10 (by default)
  Args:
    num_classes (uint): number of classes
  """
    model = CifarResNet1(ResNetBasicblock, 32, num_classes)
    return model
