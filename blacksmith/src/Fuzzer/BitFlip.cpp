...
void to_json(nlohmann::json &j, const BitFlip &p) {
  std::stringstream addr, phys_addr;
  addr << "0x" << std::hex << (uint64_t)p.address.to_virt();
  phys_addr << "0x" << std::hex << (uint64_t)p.address.to_phys();

  uint64_t phys_address = p.address.to_phys();
  uint64_t page_size = getpagesize();
  uint64_t pfn = phys_address / page_size;
  uint64_t page_offset = phys_address % page_size;
  ...
  j = nlohmann::json{
      ...
      {"addr", addr.str()},
      {"page_offset", (uint64_t)p.address.to_virt() % page_size},
      {"phys_addr", phys_addr.str()},
      {"pfn", pfn},
      {"phys_page_offset", page_offset}
  };
}
