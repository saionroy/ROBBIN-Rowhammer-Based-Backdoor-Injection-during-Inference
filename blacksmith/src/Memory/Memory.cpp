...
// Function to retrieve the physical address of a given virtual address
uintptr_t get_physical_address(uintptr_t virt_addr) {
  ...
  // Calculate the offset in pagemap for the virtual address
  size_t page_size = getpagesize();
  off_t offset = (virt_addr / page_size) * sizeof(uint64_t);

  // Seek to the pagemap entry for the given virtual address
  ...
  uintptr_t phys_addr = (entry & ((1ULL << 55) - 1)) * page_size + (virt_addr % page_size);
  return phys_addr;
}
...
void Memory::allocate_memory(size_t mem_size) {
  ...
  uintptr_t virt_addr = 0x202001adcb; // Example virtual address
  uintptr_t phys_addr = get_physical_address(virt_addr);
  if (phys_addr != 0) {
    printf("Virtual address: 0x%lx -> Physical address: 0x%lx\n", virt_addr, phys_addr);
  } else {
    Logger::log_error("Failed to retrieve physical address.");
  }
}
...
//   // Allocate memory using standard 4KB pages
//   target = (volatile char *) mmap((void *) start_address, mem_size,
...
//   if (mlock((const void *)target, mem_size) != 0) {
//     perror("mlock failed");
//     exit(EXIT_FAILURE);
//   }
...
//   uintptr_t virt_addr = 0x202001adcb; // Example virtual address
//   uintptr_t phys_addr = get_physical_address(reinterpret_cast<uintptr_t>(target));
//   if (phys_addr != 0) {
//     printf("Virtual address: 0x%lx -> Physical address: 0x%lx\n", target, phys_addr);
//   } else {
//     Logger::log_error("Failed to retrieve physical address.");
